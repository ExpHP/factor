...
impl FactorSieve {
    ...
    where
        T: MoreNumCast + Integer,
    {
        assert!(limit >= T::zero());
        FactorSieve {
            sieve: factor_sieve_simple(limit.to_usize().unwrap()),
        }
    }
}
    ...
    T: Clone + Zero + One + Integer + MoreNumCast,
{
    fn try_factor(&self, x: &T) -> Option<T> {
        if x == &T::one() {
            ...
            Some(T::from_usize(self.sieve[x.to_usize().unwrap()]).unwrap())
        }
    }
}
...
fn test() {
    ...
    assert_eq!(
        &correct[..12],
        &vec![0, 1, 2, 3, 2, 5, 2, 7, 2, 3, 2, 11][..]
    );

    // try various sizes, in particular:
    for len in vec![
        0,
        1, // Obvious edge cases (length 0, 1)
        2,
        3, // More subtle edge case (no multiples to cross off)
        9,
        10,
        11,            // End in a prime square; a composite; and a prime
