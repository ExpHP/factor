    ...
    T: Clone + Zero + One + Integer,
{
    ...
    where
        Self: Sized, // FIXME why does this require Sized?
    {
    ...
    T: Clone + Zero + One + Integer,
{
    ...
    fn first_prime(&self, x: &T) -> Option<T> {
        ...
        if x == &T::one() {
            ...
            Some(self.try_factor(x).unwrap_or(x.clone()))
        }
    }
}
...
pub mod helper {
        ...
        T: Clone + Zero + One + Integer,
    {
        ...
        if x == T::zero() {
            panic!("Zero has no factorization")
        }
        if x == T::one() {
            return One::one();
        }
...
impl<T> TryFactor<T> for TrialDivision
where
...
impl<T> SureFactor<T> for TrialDivision
where
    T: Clone + Zero + One + Integer + Shr<usize, Output = T> + MoreNumCast,
{
    fn factorize(&self, x: T) -> Factored<T> {
...
impl<T> FirstFactor<T> for TrialDivision
where
    T: Clone + Zero + One + Integer + Shr<usize, Output = T> + MoreNumCast,
{
...
impl<P, F, T> TryFactor<T> for Stubborn<P, F, T>
where
    P: PrimeTester<T>,
    F: TryFactor<T>,
    T: Clone + Zero + One + Integer,
...
impl<P, F, T> SureFactor<T> for Stubborn<P, F, T>
where
    P: PrimeTester<T>,
    F: TryFactor<T>,
    T: Clone + Zero + One + Integer,
...
impl<P, F, T> FirstFactor<T> for Stubborn<P, F, T>
where
    P: PrimeTester<T>,
    F: FirstFactor<T>,
    T: Clone + Zero + One + Integer,
...
mod tests {
    ...
    mod list_factorizer {
            ...
            pub fn compute_new(n: T, factorizer: &dyn SureFactor<T>) -> Self {
                ListFactorizer {
                    ...
                    factors: num::iter::range(T::zero(), n.clone())
                        .map(|x| factorizer.try_factor(&x).unwrap_or(x.clone()))
                        .collect(), // XXX HACK: _or()
                }
            }
        }

        impl<T> TryFactor<T> for ListFactorizer<T>
        where
            T: Clone + Integer + MoreNumCast,
        {
        ...
        impl<T> SureFactor<T> for ListFactorizer<T> where T: Clone + Integer + MoreNumCast {}
    }
    ...
    fn bench_list_trialdiv(b: &mut Bencher) {
        b.iter(|| make_list(TrialDivision, 10000u64));
    }
    ...
    fn bench_ten_8_rough_trialdiv(b: &mut Bencher) {
        b.iter(|| TrialDivision.try_factor(&TEN_8_ROUGH));
    }
