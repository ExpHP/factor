...
impl<T> PrimeTester<T> for PrimeSieve
where
    T: Eq + Clone + Integer + MoreNumCast,
{
...
impl<T> PrimeTester<T> for MillerRabin
where
    T: Eq + Clone + Integer + Shr<usize, Output = T> + Debug + MoreNumCast,
{
    fn is_prime(&self, x: &T) -> bool {
        if *x <= literal(3) {
            return *x >= literal(2);
        ...
        'witness: for witness in self.collect_witnesses(x.clone()) {
            ...
            while counter > literal(0) {
                // turn this into a for loop, I dare you  (please?)
...
where
    T: Eq + Clone + Integer + Shr<usize, Output = T> + MoreNumCast,
{
    let mut remaining = x;
...
fn test_miller_mersenne() {
    ...
    for p in vec![19usize, 31, 89, 107].into_iter() {
        // known p-values for some mersenne primes
...
fn bench_mersenne(b: &mut Bencher) {
    ...
    b.iter(|| MillerRabin.is_prime(&mersenne))
}
